...
\else % if luatex or xelatex
  \ifxetex
    ...
    \setCJKmonofont[BoldFont={FZZhongDengXian-Z07S},
                  ItalicFont={FZXiYuan-M01},
                  SlantedFont={FZBaoSong-Z04S}]{FZXiDengXian-Z06}
  \else
    \usepackage{fontspec}
  \fi
...
$endif$
$if(listings)$
\usepackage[savemem, procnames]{listings}
\lstset{basicstyle=\small\ttfamily, showstringspaces=false,
        basewidth=0.5em, breaklines}
\makeatletter
\renewcommand\lstinline[1][]{%
    \leavevmode\bgroup % \hbox\bgroup --> \bgroup
      \def\lst@boxpos{b}%
      \lsthk@PreSet\lstset{flexiblecolumns,basicstyle=\ttfamily#1}%
      \lsthk@TextStyle
      \@ifnextchar\bgroup{%
        \ifnum`{=0}\fi%
...
\newenvironment{cnabstract}{
    ...
    \vspace*{-24pt}
    \pdfbookmark[0]{\cnabstractname}{abstract}
    }
    {\par}
...
\end{cnabstract}
$if(project)$
    \newcommand\cnkeywords[1]{ {\noindent\bfseries\cnkeywordsname：}#1}
    ...
    \vspace*{2em}
    \par
    {\noindent\bfseries 合作者：}
    \begin{tabular}[t]{ l l l }
    $for(partner)$
    $partner.name$ & $partner.number$ & $partner.affiliation$ \\
    $endfor$
    \end{tabular}
    $endif$
    $if(enabstract)$
